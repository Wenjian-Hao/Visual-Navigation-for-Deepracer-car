...
def TFKNvidiaModel():
    ...
    output_2 = Dense(1, name='output_2')(dense3)
    optimizer = Adam(lr=1e-3)
    model = Model(input=inputs, output=[output_1, output_2])
    model.compile(optimizer= optimizer, loss={'output_1': 'mean_squared_error', 'output_2': 'mean_squared_error'}, metrics=['accuracy', 'accuracy'])
    return model
